...
| 18 | DomainEventStore の retry path を実際に exercise する test (現 `DomainEventStoreConcurrencyTest` の 3 ケースは catch/retry branch を hit していない) — QueryException mock 注入で 1 度だけ 23505 を投げる unit test が必要 (PR #26 review HIGH 2 follow-up) | P2 | (任意) |
| 19 | DomainEventStore retry に backoff/jitter (`usleep(random_int(...) * 2 ** $attempt)`) + retry log (`Log::warning` per attempt) 追加。PR #26 review MEDIUM follow-up | P3 | (任意) |
| 20 | `phpunit.xml` を sqlite-only から pgsql matrix に拡張、または pgsql-only smoke job を CI に追加。Next 26 開発中に 2 件 (DomainEventStore + ChangeUserRoleHandler) 発覚した「pgsql で fail / sqlite で pass」クラスを以後落とさないため | P2 | (任意) |
