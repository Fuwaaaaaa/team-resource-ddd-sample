...
| 17 | **[BUG, P0]** `App\EventStore\DomainEventStore::append` が `select max(stream_version) ... FOR UPDATE` を発行するが、PostgreSQL は aggregate と FOR UPDATE の組み合わせを禁止 (`SQLSTATE 0A000`) → Docker / 本番 (pgsql) で全 Domain Event 発火が 500。CI はおそらく phpunit.xml の sqlite override で通る。修正方針例: 同一 stream の既存行 1 つを `SELECT id FROM domain_events WHERE stream_type=? AND stream_id=? ORDER BY stream_version DESC LIMIT 1 FOR UPDATE` で row-lock → そのあと max を別 query。 unique 制約の `(stream_type, stream_id, stream_version)` があれば、楽観ロック方式 (lock 削除 + retry on 23505) でも安全 | P0 | 緊急修正 PR (Next 26 とは別) |
